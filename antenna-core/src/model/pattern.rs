...
/// (`CachedGain` (in `antenna-model`'s service layer)), because the cache hit that serves a
...
/// see `ray_trace_stub_warning` (in `antenna-model`'s `service::evaluator`) for the service-layer

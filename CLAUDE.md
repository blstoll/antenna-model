...
- **`bessel.rs`** - In-house Bessel Jₘ (pure Rust, Numerical Recipes rational approximations + two-branch recurrence), pinned by tests in both the small-argument and asymptotic branches, and since P10-perf across the **turning point** `m ≈ x` at orders up to 254. `bessel_jn_array` returns every order `J_0…J_{m_max}` from a single sweep and is what the mode integrator uses — do not mix it with per-order `bessel_jn` calls on the same path, since the two select their recurrence direction from different orders. **Known, pinned, latent:** accuracy degrades exactly at `m ≈ x` (2e-8 at x=255, 9e-3 at x=10⁴) because the Miller branch's seed offset is a flat constant where the transition width grows like `x^(1/3)`; harmless only because `MODE_M_MAX` caps served orders at 254. Roadmap unit **P14**.

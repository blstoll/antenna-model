...
- **`integration.rs`** - Aperture integration via the **Hankel / azimuthal-mode (Jₘ) integrator** (roadmap P10, landed 2026-07-15): the φ' integral is collapsed analytically (Jacobi–Anger), radial density is derived adaptively from `(D/λ, θ)` at ~2× Nyquist, and a runtime self-check flags non-convergence (surfaced as a response warning). **The self-check is not the same on both paths, and it is not total** (measured 2026-07-31, roadmap D17 → unit **P12**): the *symmetric* path runs an N-vs-2N **radial** check (`integration.rs:514-530`), but the *asymmetric* (azimuthal-mode) path checks only mode truncation, `I(M)` vs `I(M+1)` — `n_rho` is computed once at `integration.rs:545` and never verified. So on a laterally-offset or `asymmetry_factor != 1.0` feed — which is **five of the enabled feeds** — `converged = true` says nothing about the radial quadrature. Measured silent errors against the exact `antennas.yaml` parameters: **0.82 dB** (`gs_3.7m` X-band, θ=5°), **1.17 dB** (`dsn_34m` X-band, θ=0.10°), **1.23 dB** (D12's UHF fixture, θ=16°). **Not latent** — and not only a density-floor problem: at the `gs_3.7m` point the floor is not binding and the ~2× Nyquist budget is itself short. Open as P12; **do not tune radial sample counts or "optimize" this path until it lands.** The legacy 2D Simpson quadrature survives only as a `#[cfg(test)]` reference oracle. The `IntegrationParams` presets (`fast()`, `high_accuracy()`) no longer gate served correctness — the served path uses `adaptive()` and most preset fields are inert (see the docstrings in `integration.rs`).

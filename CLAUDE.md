...
- **`integration.rs`** - Aperture integration via the **Hankel / azimuthal-mode (Jₘ) integrator** (roadmap P10, landed 2026-07-15): the φ' integral is collapsed analytically (Jacobi–Anger), radial density is derived adaptively from `(D/λ, θ)` at ~2× Nyquist, and a runtime self-check flags non-convergence (surfaced as a response warning). **The self-check is not the same on both paths, and it is not total** (measured 2026-07-31, roadmap D17): the *symmetric* path runs an N-vs-2N **radial** check, but the *asymmetric* (azimuthal-mode) path checks only mode truncation, `I(M)` vs `I(M+1)` — `n_rho` is computed once and never verified there (`integration.rs:526-541`). So on a laterally-offset or `asymmetry_factor != 1.0` feed, `converged = true` does **not** mean the radial quadrature converged: at low `D/λ` the served `adaptive()` preset was measured 1.16 dB from the converged value with no warning. Latent on the enabled antennas (large `D/λ`), open in the P10 family. The legacy 2D Simpson quadrature survives only as a `#[cfg(test)]` reference oracle. The `IntegrationParams` presets (`fast()`, `high_accuracy()`) no longer gate served correctness — the served path uses `adaptive()` and most preset fields are inert (see the docstrings in `integration.rs`).

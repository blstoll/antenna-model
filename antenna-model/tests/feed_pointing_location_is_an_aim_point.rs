...
//! the API `feed_pointing_location` is the feed's *pointing* location — an Earth aim point — NOT
...
//! turn (`feed_pointing_location`, `reflector_boresight`, `vehicle_position`) into a physical feed
...
//!
//! If someone ever reintroduces the "feed_pointing_location = fixed physical location" misreading,
...
fn feed_pointing_location_resolves_relative_to_vehicle_not_absolute() {
    ...
    assert!(
        d > 1e-6,
        "feed_pointing_location must resolve relative to the vehicle: identical physical feed \

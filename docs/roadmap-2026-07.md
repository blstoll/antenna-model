...
- **Ka phase-center defocus (diagnosed → unit P7):** the remaining −3.5 dB Ka residual is
  ...
  model auto-refocus.
- **Off-axis sidelobe scope (P8 shipped; F7 PARKED 2026-07-13):** the honesty warning (P8) is
  live. The sidelobe *floor* (F7) is **parked and must not be merged** — reworking it after the
  2026-07-12 review exposed a **P0**: the harness measured "sidelobes ~8–13 dB *too low*" using
  `high_accuracy()` on the small 3.7 m dish, but the **served** path uses `fast()`, where the
  aperture integral **aliases** and off-axis gain comes out **20–35 dB too HIGH** (DSN 34-m:
  **+34 dBi at 90° off-boresight**). F7's premise was therefore inverted, and a gain-*raising*
  floor could never fire (0 of 6 real geometries). Filed as row **P10**; full evidence in
  `docs/findings-2026-07-13-off-axis-integration-aliasing.md`. **Off-axis gain on every endpoint
  is currently unusable**, and this addendum's own "sidelobes are optimistic" claim above is only
  true of the high-accuracy integrator, not of production.
...
| **P10** | **Off-axis aperture-integral aliasing (P0 CORRECTNESS).** The service computes every gain with `IntegrationParams::fast()`; beyond a few degrees the far-field integral is under-sampled and aliases, so served off-axis gain is **20–35 dB too HIGH** (DSN 34-m: **+34 dBi at 90° off-boresight**). Pre-existing; affects gain/batch/heatmap/h3 alike. Hid behind a test/production integrator gap (harness validates with `high_accuracy()` on the *small* 3.7 m dish; production serves `fast()` on dishes up to 100 m). Evidence: `docs/findings-2026-07-13-off-axis-integration-aliasing.md`. **✅ SPIKE 2026-07-13 — this is a CONTAINED REFACTOR.** The azimuthal integral has a closed form (Jacobi–Anger): the 2D integral collapses to a **1D Hankel transform**, which reproduces the 2D **exactly (Δ=0.00 dB)** where the 2D is valid, converges at θ=90° to **−33.30 dBi in ~1 ms** (vs **3184 ms** brute-force, and vs **+34 dBi garbage** from today’s `fast()`), and drops the cost class **O((D/λ)²) → O(D/λ)**. Correct off-axis gain becomes **~5× cheaper than the wrong gain we ship today**; the <100 ms budget stops being a constraint. | Fix the integrator (Hankel / azimuthal-mode expansion) — brute-force density is structurally infeasible | **Hankel refactor of `integrate_aperture`** (signature unchanged; evaluator/cache/heatmap/h3 untouched). **NOTE the spike RESHAPED this row:** there is no longer a *numerical* validity wall, so `θ_valid` becomes a **physical** boundary (where idealised PO stops matching reality) — a different question from where the quadrature breaks. **All six sub-decisions D-1..D-6 DECIDED 2026-07-14** (see the resolution block atop the P10 work-unit decision table): D-1 azimuthal-mode expansion (confirmed *required* — the served DSN/gs feeds are laterally offset, so a symmetric-only integrator leaves the production configs aliased); D-2 keeps P10 to the correct integrator + honesty warning, with the statistical substitution split into a separate F7-redesign unit; D-3 ship an interim honesty fix on `main` first; D-4 single adaptive path (retire presets); D-5 fix higher-order too, flag ray-tracing; D-6 ~2× Nyquist + convergence self-check. | **Decided (P0) — ready to execute** | Filed 2026-07-13; spike 2026-07-13; decisions 2026-07-14 |

...
| **2 — Safety & operational correctness** ✅ **DONE 2026-07-23** | Config promises kept; bounded work; honest lifecycle. | ✅ Oversized → 413 (S1); slow → 504 timeout (S2); integration has a wall-clock budget (S3); concurrency capped → 503 + Retry-After (S4); readiness/fail_fast/shutdown real (S5); H3 validator complete (S6). S7 superseded by C8. (Coordinate-ambiguity handling moved to C8, which removes the ambiguity instead of warning about it.) |

...
- **`bessel_jn` is inaccurate exactly at its turning point `m ≈ x`, and nothing bounded it until
  now** (filed 2026-08-01 by P10-perf as unit **P14**). The downward (Miller) branch starts a flat
  ...
  **Latent, and bounded by a single constant:** the only caller reaching that branch is the
  ...
  accuracy cliff. Pinned by
  `jn_turning_point_accuracy_degrades_far_above_the_served_order_ceiling` so it cannot regress or
  silently become relevant.

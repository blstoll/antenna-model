...
- **The calibration pipeline's accuracy claims are now verified end-to-end, with caveats**
  ...
  **base preset** axis remains open and is the larger of the two: `calibrate` builds from
  ...
  density (`model/integration.rs:526-541`). That is a served-path convergence defect rather
  than a pipeline-consistency one, so it is routed to the P10 family; see the D17 unit for
  the full measurement. The service side is otherwise unaffected throughout: the four enabled
  antennas are uncalibrated design-spec entries that load no correction surface.

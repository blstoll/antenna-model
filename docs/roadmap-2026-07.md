...
- **Docs actively mislead.** CLAUDE.md misstates the sprint status and the correction-surface
  ...
  ~~`architecture.md` lists model files that do not exist.~~ **Its §11 repository-structure
  tree was rewritten against `ls` reality for all three crates by D4 (2026-08-13); §10.1's
  dependency block is now marked as the original design sketch, with the three shipped
  manifests named as authoritative.** The design doc still describes an
...
- **Structural debt.** ~~`calibrate` depends on the entire `antenna-model` crate, so building
  ...
  tree;~~ **both resolved by D4 (2026-08-13). The crate split put the physics engine and
  artifact layer in `antenna-core`, and `calibrate`'s normal dependency tree went 9 → 1 on
  `poem|h3o|utoipa|dashmap|lru` — every web-stack line gone, the survivor `lru` arriving
  through calibrate's own `aws-sdk-s3`. `antenna-model` is retained as a *dev*-dependency
  on purpose, because the CLI e2e tests serve artifacts through the real service path (C13);
  a package-scoped `cargo build -p calibrate` plus a `cargo tree` assertion in the gate is
  what keeps it dev-only. The two-`ndarray` half was already stale by the time D4 ran — the
  tree carried a single 0.16.1 — and D4 removed `ndarray` from the service and core
  outright as a dead dep (calibrate still uses it, via `argmin-math`)**;
  ~~the deprecated 612-line `calibrate/src/serializer.rs` writes binary artifacts the
  ...
  **resolved in two steps — the unloadable binary writer went with the 2026-07-18 postcard
  ...
  ~~a 3.1 MB `tarpaulin-report.html` is committed at the repo root~~ — **false, and it
  appears never to have been true: checked 2026-08-16 under D6, `git log --all --
  tarpaulin-report.html` is empty, the path is untracked, and `.gitignore:27` already
  carries the pattern. What existed was an untracked local file on one working machine,
  invisible to `git status` precisely because it is ignored**; no property-based tests
  exist despite CLAUDE.md claiming
  them; ~~no `.bin` calibration artifacts exist anywhere~~ — **still true and now
  deliberate: D9 decided docs-only on 2026-08-16, so this is a design position rather
  than a gap** (the four `antennas.yaml` entries
...
vacuous); **D13** (✅ 2026-07-31) — a real-data boresight-mode test from the NTIA multi-frequency gain
...
| **1 — Prediction correctness & physics scope** ✅ **DONE 2026-07-18** | No unexplained numbers on the live path; scope decisions recorded. | P1–P3 decisions in the register; spillover applied on the uncalibrated path with calibrated outputs unchanged (P1) and artifacts stamped with a physics-model version (P1b); f/D fails loudly (P4, 2026-07-17); single G/T implementation (P5, 2026-07-17); domain-contract open items current (P6, 2026-07-18). Beyond the original criteria, the phase also absorbed P7 (auto-refocus), P8 (off-axis honesty warning), P10/P10-tail (off-axis integrator), P11 (unified uncorrected-physics gate), and F7/F8 (sidelobe floor + data) — see the work-units doc. **P10-perf ✅ DONE 2026-08-01** — promoted the same day from latency fast-follow to a served-coverage item (after P12's φ' fix, steered in-scope geometries could exhaust S3's wall-clock budget and serve a 504), then closed: a new in-house mixed-radix FFT for the φ' transform (`model/fft.rs`), a single-sweep `Jₘ` ladder (`bessel_jn_array`), and — the change nobody had scoped, because nobody had measured the profile — three hoists in the aperture-plane function that had become ~79 % of a sweep once the first two removed the `×M` terms. Served mode path **2.4–7.4× cheaper** with **no physics change** (`PHYSICS_MODEL_VERSION` still 7; every P10/P12 anchor and both 2D-Simpson-oracle cross-checks unmoved). The coverage hole is closed: the ~5° steer that was 22.3 s isolated / 31.6 s contended, breaching S3's 30 s budget inside the repo, is now **4.0 s**. It also shrank D18's slow tier from nine physics tests to three, filed **P14** (`bessel_jn` turning-point accuracy — latent behind `MODE_M_MAX = 254`; **✅ closed 2026-08-01**, `PHYSICS_MODEL_VERSION` 9 — the Miller start offset now scales as a derived `12·x^(1/3)` and turning-point closure is ~3e-16 flat instead of growing without bound in x, so raising that cap is now safe), and inverted **P13**'s premise: the radial pre-gate's probe leg now saves ~33 % of a full check leg where it once saved ~80 %, so P13 should expect to delete the pre-gate rather than validate its constant — **which P13 did on 2026-08-01 (`PHYSICS_MODEL_VERSION` 8), after finding a second and stronger reason: this unit's own physics-neutral `next_fast_len` φ' resizing had already pushed the pre-gate's fitted safety factor past breaking (worst *passing* probe-to-total ratio 43.5× against a constant of 32).** Wide-angle Ka is 3.8× faster but still 559 ms at θ=90° (~748 ms after P13's honest second leg) — inside S3's budget, above the <100 ms p95 target; the remaining cost is ~85 % aperture-plane evaluation, a different problem. |
...
| **3 — API contract quality** | A client can trust the spec and the error contract — finalized once, then frozen. | One error vocabulary, JSON bodies, one status-code policy (C2–C4); `/h3-heatmap` documented in openapi + api-documentation with a verified example (C1, 2026-07-25); warnings no longer lost on a `/h3-heatmap` cache hit (C10, 2026-07-25); prose examples under a schema drift guard (C11, 2026-07-25); `loss_db` referenced to the grid peak on both heatmap endpoints (C9, 2026-07-26); **C8 contract finalization landed 2026-07-28 — all 4 stages complete**: ✅ stage 1, the aim-point field renames (`feed_position` → `feed_pointing_location`, `GeometryInfo.feed_offset_meters` → `physical_feed_offset_m`, `FeedInfo.position_offset` → `design_feed_offset_m`), landed 2026-07-26 on branch `feat/c8-stage1-aim-point-field-rename` with no computed value moved; ✅ stage 2, `coordinate_system` required (2026-07-27, branch `feat/c8-stage2-required-coordinate-system`) — the magnitude heuristic, `ECEF_THRESHOLD_M` and the `coordinate_ambiguity_warnings` plumbing are deleted, `Position3D::new` is replaced by `ecef()`/`geodetic()`, an untagged position is a 400 naming the field, and no computed value moved; ✅ stage 3, typed warnings (2026-07-27, branch `feat/c8-stage3-typed-warnings`) — `warnings` is `Vec<ApiWarning>` on all three response types, backed by a closed 14-variant `WarningCode` enum with an openapi/docs drift guard; the C2-deferred batch per-item failure shape became a typed `error` field reusing the `error_codes` vocabulary; no computed value moved; ✅ stage 4, endpoint coherence + spec completeness, **landed 2026-07-28 on branch `feat/c8-stage4-endpoint-coherence`** — the `/heatmap` H3 grid-type stub removed (`h3` grid_type is now a 400, an unknown serde variant, per C2), the producerless `not_implemented` error code retired (vocabulary is 10 codes), and every antenna-endpoint openapi schema audited (deliberately **not** hand-fixed — deferred to C7, which was re-scoped from a hand-maintained spec to utoipa auto-generation from the Rust types). **C7 landed 2026-07-29** (commit `5cf26e5`, "contract frozen"): `openapi.yaml` is now generated from the Rust types/handlers via `utoipa`, pinned byte-for-byte by `tests/openapi_spec.rs`, with route coverage pinned by `tests/openapi_routes_match.rs`. Stage 1 also filed three units it was not chartered to fix — C12 (`rmse_db`/`r_squared` documented as omitted but emitted as `null`; **done 2026-07-28**, decided in favour of declaring the `null` — see register row C12), C13 (`design_feed_offset_m` origin is vertex- vs focus-relative depending on the producer — **✅ fixed 2026-08-02 under D14**, which stopped it being latent by serving a full-mode artifact for the first time and measuring what the wrong frame costs: the feed lands at `z ≈ 2f` and boresight gain drops **41.09 → 13.83 dBi**, so it was a served-gain defect, not just a reported-field one; `calibrate` now writes the offset focus-relative and each producer has its own guard), C14 (openapi's feed-listing surface disagrees with the service; **superseded 2026-07-28** by C7's utoipa generation — see register row C14). **✅ Phase 3 complete — the contract is frozen behind C7's generated-spec drift guard (2026-07-29).** |
| **4 — Structure, debt, docs** | The codebase stops accumulating the debt classes found in this review. | ✅ Legacy serializer gone (D1, 2026-07-29 — `serializer.rs` → `sidecar.rs`; the `CalibrationArtifact` wrapper and its dead constructor/summary/helpers deleted, the two JSON exporters now take the metadata and report directly; the binary writer had already gone in the 2026-07-18 postcard migration); ✅ version axes documented+validated (**D2, 2026-07-30**, branch `fix/d2-artifact-version-axes` — the ANTC header `u32` is the **container** axis, readable before the decode and the only guard against a file this build cannot parse; `metadata.format_version` is the **schema** axis, readable only after the decode and the only guard against a payload that decodes cleanly and *means something else* — the class postcard's positional encoding makes real. `format_version` went from a `warn!`-only string compare to a parsed `MAJOR.MINOR`: foreign MAJOR and unparseable stamps are hard errors, a differing MINOR warns; the check now runs before this build's validation rules touch a field. Neither version bumped. D1's inherited finding — boresight mode writing a bare headerless `postcard::to_allocvec`, with no version stamp to check and no CRC — is fixed structurally: `write_antc_artifact` moved into the library as the tool's **only** artifact writer, taking magic/version/header-length from the loader's public constants, so the two producers cannot drift apart again. Both-producer round trip pinned by the new `cli_boresight_mode_e2e.rs` beside D12's full-mode file, plus a bit-flip test asserting the CRC now catches corruption a headerless artifact could not); 3D→4D bridge round-trip-tested; ✅ **crate split done (D4, 2026-08-13** — three crates; `antenna-core` holds the physics engine and artifact data layer, `calibrate` no longer compiles the web stack (normal dep tree 9 → 1 on `poem|h3o|utoipa|dashmap|lru`, the survivor `lru` arriving via calibrate's own `aws-sdk-s3`), `antenna-model` stays a *dev*-dependency so the CLI e2e tests keep serving artifacts through the real service path, and the two package-scoped properties a workspace build hides — core's `openapi`-OFF configuration and calibrate's own feature declarations — are now compiled and asserted in the gate**); design docs truthful; property tests in CI; ✅ **D26 (2026-08-13** — the review of the merged D21, filed and closed the same day: one served-output defect plus four soundness gaps in the angular-resolution code. The served defect needed a **second half the filing had not named** — a silent `.max(0.0)` clamp on the exported elevation extent collapsed a legal negative-E-cone cut to `(0.0, 0.0)`, so a full-mode artifact over thousands of measurements reported `is_boresight_only()` and the service applied **no correction at all** while every other health signal read normal; but widening the range alone would have left the correction *surface* unreachable, since its elevation axis is the fitted E-cone axis and the served elevation is a polar angle that is never negative — trading “no correction” for “a correction read at a clamped edge”, which is worse because the first is at least visible as `correction_applied: false`. Fixed as a **convention**: the parser reflects `(φ, −θ)` onto the identical direction `(φ + 180°, θ)` before anything downstream sees it, pinned physics-preserving to **3.2e-6 dB** on the asymmetric branch, and the export now *refuses* an out-of-convention extent — a clamp cannot tell “already correct” from “silently truncated”, which is how it survived. **No number in the tree moved**: no fixture carries a negative cone row, D14's generator having independently arrived at the same half-planes-in-clock convention, so the filing's “this moves the D14 e2e” was wrong. The other four: the assessed diameter now cannot diverge from the stamped one because the parameter was **deleted** rather than validated (the C13/D23 invariant, two lines apart in the same function); `widest_knot_gap` refuses a NaN gap it used to silently discard — reporting a *better*-resolved verdict from corrupt input; `min_knot_spacing_*` is validated, so D25's reachability premise is enforced rather than assumed; and the worse of `INFINITY`'s two meanings was deleted rather than renamed, making `INF/INF = NaN` into artifact metadata unreachable by construction**); **plus the two correctness-class defects D1 surfaced in the `calibrate` pipeline and could not fix in charter — ✅ D10 and D11, both landed 2026-07-29** (branch `fix/d10-d11-calibrate-correctness`), ahead of the CLI integration-test work as required. **D10:** the validator refitted every cross-validation fold with `CorrectionSurfaceParams::default()`, validating a surface with ~2× the knots at 1000× weaker regularization than the artifact being blessed. The nested CV also proved *worse than filed* — not merely unrequested but **unbounded recursion**: `cross_validate` refits each fold with the same fold count, re-entering itself until the shrinking training set trips the `(spline_order+1)³` minimum, so **cross-validation could not complete on any input** (256-point fixture: 205 → 164 → 132 → 106 < 125, fail). Fixed at both layers through one `without_nested_cross_validation()` helper; the reported CV RMSE moved **+0.041 dB worse**, which is the fix working. **D11:** `MeasurementPoint::validate` rejected any row below −20 dB/K — a boresight figure — silently dropping legitimate sidelobe measurements via `eprintln!`. On a realistic ITU-R S.580 pattern that discarded **~78% of every grid**, retaining only the near-boresight population regardless of grid density. `validate` is now physicality-only (and finally rejects non-finite values, which it never did — `NaN` frequency, temperature and G/T all passed before); atypical G/T became a `DataQualityReport` warning with a G/T-range readout; drops now report through `tracing` at WARN with an accurate count and a bounded sample. Two further findings were filed from the pair, neither in charter: `detect_outliers` runs its modified Z-score on **raw G/T rather than residuals**, so it now flags 40% of a full pattern purely because the data got wider; and `create_sample_csv` generates a `41.5 − (θ/5)²` rolloff bottoming out at +5.5 dB/K, too shallow to have ever exercised the gate it sat just inside. That integration-test work is itself now specified (filed 2026-07-29 after the calibration-data assessment, §1 addendum): **✅ D12** — perturbed-truth synthetic CLI e2e test with known-answer recovery assertions, **landed 2026-07-30** (branch `feat/d12-calibrate-cli-e2e`): the built `calibrate` binary now runs end to end (parse → fit → validate → artifact), the artifact loads through the *service's* loader, model-only RMSE 1.3071 dB drops to corrected 0.9756 dB (25.4% improvement), and four interior probes recover an injected bias within 0.65 dB (worst case 0.5928 dB, weakened by the fixture's underdetermination — see below). Filed two findings and fixed one flake (D11's log-capture tests, ~20% intermittent under parallel execution). Finding 1, the correction-surface upper-edge collapse, was **fixed 2026-07-30 by D15** (`a866cfb`) — and D15 corrected its characterization: D12 filed it as a served-path risk, but the served path was never itself wrong. The 4D interpolator is correct; artifacts were *fitted* wrong and the service faithfully served the bad coefficients. Finding 2, `--tune-parameters` crashing in argmin's Nelder-Mead, is **✅ fixed by D16, 2026-07-31**. **✅ D13 — real-data boresight-mode test from the NTIA frequency sweeps, landed 2026-07-31** (branch `fix/d13-boresight-correction-flat-axes`). Two committed real-data fixtures rather than the one filed, because the primary candidate covers only one branch: **Andrew 43998, 10 m, six frequencies 3700–6425 MHz** — tuned physics reconciles all six *published* gains at **0.0828 dB RMSE** (0.4040 dB untuned), below the 0.5 dB correction-fit threshold, so it covers the **uncorrected** branch; and **Scientific-Atlanta 8002A, 10 m, five frequencies** — whose published Tx-band gain (50.8 dBi at 6175 MHz, aperture efficiency ~0.28 against ~0.63 at 3950 MHz) cannot be reconciled with its Rx-band gains by any single-reflector model, so the residual stays at 0.6214 dB and a correction *is* fitted, attached, and **reached on the served path**. That is the Rx/Tx different-feed caveat the scope anticipated, showing up loudly rather than quietly, and it is what buys real-data coverage of the **corrected** branch. Measured tolerances, all named constants with the measurement beside them: Andrew served-vs-published worst **0.483 dB** (tolerance 0.75); Andrew with the spillover term removed worst **0.157 dB** (tolerance 0.25); SA served-vs-published *with the correction applied* worst **0.055 dB** (tolerance 0.25). 10 tests, 0.94 s debug. Supporting production change: `BoresightMeasurements::from_csv` now skips `#` lines so a committed fixture can carry its provenance and assumptions and still be runnable **as committed** (and reports the real file line, not a record ordinal, once a provenance block sits above the data) — not a step toward harmonizing the two parsers; the drop-vs-fail difference is untouched and now stated in the doc comment. **✅ Fixed 2026-07-31 by D17 (same day) — a boresight artifact with no correction surface was served with a spillover loss its own calibration never saw:** calibrate's objective runs under `IntegrationParams::default()` (`apply_spillover = false`) while the service sets `apply_spillover = calibration.physics_is_uncorrected()`, i.e. **on** for exactly those artifacts. Measured **−0.326 dB** constant for Andrew and **−0.953 dB** at the SA fixture's tuned q of 0.70; at boresight `default()` and `adaptive()` agree to 1e-4 dB and the F7 floor contributes ~1e-4 dB, so spillover is the *entire* gap, and removing it recovers the tuner's own fit exactly. Same defect class as D16's filed finding, one seam further out — the divergence is between calibrate and **the service**. Full mode is unaffected in practice (it always attaches a correction, so the service turns spillover off there too). **✅ Closed the same day by D17**, which was filed and landed on 2026-07-31: `calibrate` now tunes under the gates the service will apply to the artifact it is about to write, through one shared setter (`IntegrationParams::with_uncorrected_physics_gates`) that every producer of a served gain now goes through — five call sites across five files, carrying three distinct decisions. One deliberate non-user remains and is documented on both ends: the evaluator's ideal-reference computation sets `apply_spillover` from what the actual evaluation *applied* rather than from the predicate, because the model layer can leave the flag on and fold in no spillover, and matching the predicate there would put a one-sided bias into `loss_db`. The circularity — the gates depend on whether a correction is attached, which depends on the residuals, which depend on the gates — is resolved by running the boresight tuner in up to two passes: pass 1 under the gates a correction-free artifact is served with, which also decides the branch; pass 2 under the gates-off model only if that branch needs a correction. **Andrew's worst served-vs-published error fell 0.483 → 0.1813 dB**, and the served residual RMSE now equals the RMSE the artifact reports (0.1065 dB, to four decimals) — the reported figure *rose* from 0.0828 dB, which is the fix working: the tuner can no longer fit a spillover-free model. SA 8002A is unchanged, its residuals crossing the threshold in pass 1 so pass 2 reproduces the old single-pass result exactly. The test that pinned the defect became the standing guard against it. **D17 filed one finding it did not fix, and it is larger than the one it closed:** the *density* axis of the same question is still divergent — `calibrate` builds from `IntegrationParams::default()` (radial floor 32) and the service from `adaptive()` (floor 16), and on D12's own fixture geometry those return **−50.7668 vs −49.6090 dBi** at θ=16°/600 MHz (with `high_accuracy()` at −49.5426, agreeing with `default()` to 0.066 dB) — **1.16 dB, with `converged = true` and no warning**. Source-confirmed mechanism at `model/integration.rs:526-541`: on the **asymmetric azimuthal-mode path** the runtime self-check compares mode truncation `I(M)` vs `I(M+1)` **only** — `n_rho` is computed once and never verified, unlike the symmetric path's N-vs-2N check. So CLAUDE.md's "never silent" convergence claim holds on the symmetric path and not on the mode path. Latent on the four enabled antennas (their asymmetric feeds sit at D/λ ≈ 97–3600, so the floor binds only within a couple of degrees of boresight where the integrand is smooth); live in the calibration fixture (D/λ = 16, `asymmetry_factor` 1.1), which is where it surfaced. Left to the P10 family — it changes served values on every antenna and is a "the served preset is under-converged and says otherwise" defect, not a calibrate/service consistency one. D13's two blockers were cleared earlier the same day on the same branch: `fit_frequency_correction` built single-layer axes over `order` equal knots, so every boresight artifact carrying a frequency correction was rejected outright by the service loader — the whole boresight-plus-correction path was dead on arrival and D2 could only pin its framing on the no-correction branch. The collapsed axes are now *flat-but-valid* via one shared `artifact_export::flat_axis(lo, hi, order)`, extracted from full mode's temperature-axis construction so the two producers cannot drift; shape `[1, 1, N, 1]` → `[4, 4, N, 4]`; the known-defect pin inverted to assert acceptance, and `cli_boresight_mode_e2e.rs` gained a rippled fixture that drives the correction-carrying branch end to end. Merely lengthening the degenerate vectors would have satisfied the loader while leaving the evaluable span empty — the D15 silent-zero failure mode — so the tests assert the served correction is materially nonzero, not just loadable. **A second blocker surfaced while fixing the first and was ✅ fixed the same day**, maintainer-decided: boresight coverage recorded `azimuth_range = (0, 0)`, but boresight is the *pole* of the (azimuth, polar-angle) system, where azimuth is degenerate — every value names the same direction, and `atan2` on float noise returns any of them (measured 63.43° on a realistic ECEF geometry aimed exactly at the boresight point). That encoding constrained a coordinate carrying no information and so **rejected the very point it described**: `is_in_coverage` skipped the correction, and a loadable boresight artifact served raw physics while reporting `PartiallyCalibrated` and carrying its correction — every observable healthy except the one nobody asserted. Boresight coverage is now an on-axis **cone** (azimuth unconstrained, `elevation ≤ BORESIGHT_COVERAGE_CONE_DEG = 0.01°`, sitting two-plus orders above the `acos` noise floor and one order inside the narrowest realistic HPBW), mirrored into `validity_ranges`. `CalibrationCoverage::is_boresight_only` moved with the encoding — it would otherwise have reported `false` for boresight artifacts through the public `coverage.is_boresight_only` field — and now tests the elevation cone alone, which also keeps legacy `(0,0)` artifacts correct. The served path is pinned end to end by a test asserting `correction_applied` *and* an exact gain shift. **Filed, not fixed:** `is_in_coverage`'s azimuth clause is meaningless at the pole in general, not just for boresight artifacts — any coverage whose elevation range includes 0 contains the pole. Deliberately left to a separate unit, because fixing it under D13 would mask rather than express what a boresight artifact covers. **✅ D14 — real-anchored full-mode artifact, landed 2026-08-02** (branch `feat/d14-cr159703-real-anchored-artifact`), after **D19 + D20** cleared it (filed 2026-08-02 out of D15's excluded scope: adaptive knots landing on the axis bounds, and a data-sufficiency check that tests `(order+1)³ = 125` where the binding quantity is the 960-coefficient count). The NASA CR-159703 1.22 m dish's digitized H- and E-plane cuts anchor a 3240-row model-filled grid (`calibrate/src/bin/cr159703_grid.rs`, which echoes its nine fabrications on every run); the real `calibrate` binary fits it; and the artifact is then **served through `compute_gain_from_request` — the first time any full-mode artifact in this repository has been served**. That is what makes the unit worth more than its assertions: it immediately surfaced **C13**, filed in July as a reporting inconsistency and latent only because nothing served a `.bin`. It is not a reporting inconsistency — `calibrate` wrote the feed's design offset vertex-relative while the service adds it to an already-vertex-origin steering position, so the served feed sat a full focal length off focus and **boresight gain read 13.83 dBi instead of 41.09, −27.3 dB**. Fixed here, with a guard per producer. Measured afterwards: served boresight **41.3065 dBi against the report's published 41.4** (−0.0935 dB, inside the anchor's own 0.5 dB uncertainty); the 19 digitized peaks reproduced at **3.19 dB RMS against 11.58 dB uncorrected (3.6×), closer at 17 of 19**; the fit reproduces the fill at 0.0272 dB and calibrate's model-only RMSE equals the generator's injected-residual RMS to four decimals (11.0266 dB), which pins D17's question one seam further out. `scripts/generate-cr159703-artifact.sh` is D9's worked exemplar. **Three findings filed, not fixed:** **D21** — the shipped 2° cone knot floor cannot resolve this antenna's 1.16° lobe period, so the digitized peaks deviate from the smoothest representable curve by up to 8.42 dB and *that*, not fit error, is the 3.19 dB residual (in-sample RMSE structurally cannot see it, because the grid is sampled no finer than the knots); **D22** — cross-validation cuts folds as contiguous slices of a grid-ordered file, so the edge folds score a frequency extrapolation nobody asked for (10.07 / 0.56 / 0.12 / 0.64 / 10.86 dB against an in-sample 0.027 dB); and, from the review pass, **D23** — the artifact has no field for `asymmetry_factor`, so on `GroundStation_13m` (1.05) or `UHF_Array_Element` (1.1, D12's own fixture class) the correction is fitted against an asymmetric illumination and served on a symmetric one. **The review pass also caught the version axis:** C13 changed what an existing field *means* without moving a byte, which is precisely the case the schema axis exists to reject, so `CALIBRATION_SCHEMA_VERSION` is now **3.0** and pre-fix artifacts are refused rather than served 27 dB low — and three producers stamping the literal `"2.0"` instead of the constant, which would have drifted straight past any bump, now use it. Four smaller review findings were fixed in place: the e2e binary moved to the slow tier (a per-*test* 10 s marker cannot see a per-*binary* cost that nextest pays once per test), a non-positive digitized uncertainty is rejected at entry instead of becoming `inf` weights and NaN coefficients, the constants the test restates from the generator are pinned against its summary JSON, and the script's `--validate`/`--metadata` path — which both files claimed was pinned and which no test ran — now is. **✅ Two of D14's three filings closed 2026-08-03 — D23 and D22, the correctness-class pair; D21 remains open.** **D23:** the artifact gained `feed.asymmetry_factor` and both version axes moved (schema **4.0**, container **3**) because this time the *layout* changed, not just a meaning. The measurement the filing did not have: worst **1.20 dB** on `UHF_Array_Element` (cone 14°, 700 MHz) and 0.60 dB on `GroundStation_13m`, against **+0.0003 dB at boresight** — a φ-dependent error, which is precisely why C13's pass two lines away in the same function did not see it, and the reason the served-path guard carries a negative control requiring the served gain *not* to match the symmetric default. Maintainer decided asymmetry is a **declared** design property rather than a tuned one: it is horn geometry, and boresight sweeps (the only data the tuner runs on) carry 0.0003 dB of signal about it, so tuning it would fit noise. A second seam closed with it — boresight `compute_predictions` hardcoded `.asymmetry_factor(1.0)`, so the tuner minimised against a model the artifact could not describe, the same rule D17 established for the integration gates. The bump itself exposed two version-literal defects: a loader test that built its "unsupported" artifact from a hardcoded `3` and so became an assertion that this build rejects its own artifacts (it failed loudly, the lucky direction — a test asserting *acceptance* of a literal goes green while testing nothing), and a **fourth** hand-rolled ANTC writer, in a test, carrying a literal `2u32` that D2's single-writer rule was supposed to have eliminated. **D22:** folds are now strided (`i % K`) with the reason in the code, per-fold RMSEs are surfaced rather than a mean that hid a 100× spread, and a fold whose training split cannot be refitted is **recorded rather than fatal** — since D20 that abort let `--validate` *remove* an artifact the same command without it produces. The mean is taken over the folds actually scored; dividing by `num_folds` would have made cross-validation report a better number the less of it ran. D14's artifact re-measured: **10.07 / 0.56 / 0.12 / 0.64 / 10.86 → 0.029 / 0.031 / 0.031 / 0.060 / 0.046 dB** against an in-sample 0.0272, taking the worst fold from 370× that figure to 2.2×; the known-defect pin in `cli_full_mode_real_data_e2e.rs` was inverted exactly as its own comment instructed. **✅ D21 closed 2026-08-04, the last of D14's three filings.** Maintainer took **option 2**: every full-mode fit now measures what its knots resolve against `λ/D`, warns when it falls short, and records the figures in the run output, the `--metadata` sidecar and the artifact itself (`CalibrationMetadata.angular_resolution`). Nothing on the served path branches on it — it exists so an artifact can state a limitation **its own in-sample RMSE structurally cannot show**. The unit is worth more for what it corrected in its own filing than for the field it added. **First:** the filing named the 2° cone spacing floor as "the binding constraint", but the six requested cone knots land on 2/4/6/8/10/12° only because six knots over a 0–14° span want that spacing anyway — `num_knots_econe = 6` binds at the same point, so **option 1 as written ("derive the angular knot floors from `λ/D`") would have changed nothing on this artifact**. That is also why the shipped assessment reads the *delivered* knot vectors rather than the requested params: the floors give the wrong answer on both axes, in opposite directions. **Second:** the clock axis is the **worse** of the two by 5× — 0.119 knots per lobe period against cone's 0.577 — delivering **40°** against its own 5° floor (the floor never engages; the knot count does), and its requirement *tightens* off-axis rather than being uniform, because traversing φ at polar angle θ crosses an arc of `sin θ`, giving `Δφ = (λ/D)/sin θ`. A single absolute floor assumes exactly the opposite. The bump moved **both** axes (schema **5.0**, container **4**) and is the first here that **fixes no wrong number**: every 4.0 artifact means what it said and no consumer reads the new field, but postcard is positional, so a 4.0 payload is short by the `Option` discriminant. `MIN_KNOTS_PER_LOBE_PERIOD = 2.0` is **derived, not fitted** (P13's rule) — it is the Nyquist criterion, so there is no margin to assert. D14's `ANCHOR_RMS_BUDGET_DB` and `MIN_ANCHORS_IMPROVED` are deliberately unmoved: the exit criteria tighten them only if option 1 is taken, and it was not. **Filed D24 in its place** — see the risk entry below: "option 1 as the real fix" was being carried in two documents as a settled conclusion when it is neither proven nor currently testable. **✅ D9, D6, D8, D25 all closed 2026-08-16, in one pass.** **D9** decided docs-only (no `.bin` in-repo) and re-verified D14's generation script end to end — every documented reference figure reproduced exactly. Its README half was far larger than chartered: the file advertised ECEF/geodetic auto-detection **deleted by C8 stage 2**, documented a pre-3D `POST /api/v1/evaluate` API with a `g_over_t_db` field that does not exist, and named two config files that do not exist (`service.toml`, `antennas.toml`) — the first of which **CLAUDE.md repeated**. Default state re-measured against a *running service*: 5 enabled antennas, 11 feeds, `test_simple` disabled as a dev fixture. **D6** put the AWS subtree behind the off-by-default `s3-input` feature — calibrate's normal graph **240 → 80 unique packages**, 25 of them `aws-*` — with a fourth dep-graph invariant guarding it, falsified deliberately before being trusted; both feature states are now clippy-checked in the gate and in CI. Review caught two errors in this row's first draft, both now fixed at source: the figures were raw `cargo tree` line counts (which repeat shared subtrees) rather than unique packages, and the claim that gating shrinks `cargo audit`'s surface is **false** — audit reads `Cargo.lock`, where an optional dependency still appears. The gate now computes the figure it prints instead of carrying it in a string. **D8** deleted the dead mesh-transparency path. **D25** made the JSON sidecar *name* non-finite values instead of writing an unparseable `null`, taking option 1 over the unit's own recommended option 2 — refusing to write would have rejected a legitimate artifact, since `INFINITY` there is D26's deliberate best case, not a missing measurement. **Three of the four units had stale premises**, each corrected in place: D6's "committed" tarpaulin artifact was never committed in this history; D9's antenna counts had gone stale twice; D25's option ordering predated D26. **Filed D30** — the shipped ECEF request examples are geometrically invalid (a 422, and a batch 200 with every item failed), which both example guards are blind to because they check that payloads *deserialize*, never that they *compute*. Phase 4's remaining open units: D3, D5, D7, D18 task 3, D24 (blocked), D30. |
...
| **P12** | **The azimuthal-mode integrator never verifies radial convergence** (filed by D17 2026-07-31, promoted out of P10-perf the same day). The symmetric branch runs an N-vs-2N radial check; the asymmetric branch checks azimuthal mode truncation only, so `converged = true` asserts nothing about the radial quadrature on every offset/asymmetric feed — five of the enabled feeds. Measured silent errors of **0.82 dB** (`gs_3.7m` X-band, θ=5°, floor *not* binding), **1.17 dB** (`dsn_34m` X-band, θ=0.10°) and **1.23 dB** (D12 UHF fixture, θ=16°). Two sub-defects — floor too low, *and* the ~2× Nyquist budget short on the mode path — one root cause: nothing checks. **Two coupled calls.** **D-A:** what form does the radial check take? A full N-vs-2N triples mode-path cost, on the path P10-perf exists to make cheaper (`dsn_34m` Ka is already ~3.3 s at θ=90°). **D-B:** is `adaptive()`'s `min_rho_points: 16` simply wrong against `default()`'s 32? | D-A: (i) full N-vs-2N / (ii) dominant-mode subset check / (iii) validated budget + margin, checked in tests not per-call. D-B: raise the floor to 32 / leave it | **D-A: (ii) dominant-mode subset**, with (i) built first as the reference that grades it — never ship the cheap check without the honest one to measure it against. **D-B: raise to 32** — cheap, independent, and it closes D17's leftover calibrate-vs-service preset divergence as a side effect; explicitly *not* the fix for this unit, since it does nothing for the `gs_3.7m` case. Sequencing: **land P12 before P10-perf** (correctness ordering), but make D-A with P10-perf's speedups on the table. **OUTCOME 2026-07-31 — both calls were revised by task-1 measurement, then implemented (see the P12 unit): D-A =** return the fine (2N) leg + refine until converged, with the `{0,1}` subset as a *pre-gate only* where a full check leg is expensive (`PHYSICS_MODEL_VERSION` 6); **D-B =** floor raised to 32. Validation exposed a second silent cap — the φ' clamp, **+82 dB** wrong on a routine ~5° steer — fixed in the same unit (version 7), re-keying both effort caps to the PO scope boundary (0.5f). **Follow-ups triaged 2026-08-01:** P10-perf promoted to a served-coverage item (steered in-scope geometries can now 504 against S3's budget); **P13** filed to validate-or-retire the two empirical constants the fix rests on (`RADIAL_PRE_GATE_SAFETY = 32`, `RADIAL_PROBE_MODES = {0,1}`), sequenced after P10-perf. **✅ P13 DONE 2026-08-01 — both constants RETIRED, `PHYSICS_MODEL_VERSION` 8.** The pre-gate was deleted rather than validated, for two independent reasons either of which sufficed. (1) **The safety factor never bounded what it existed to bound:** a θ × D/λ sweep measured the worst *passing* probe-to-total ratio at **43.5×** against the constant's 32 — on `dsn_34m` Ka θ=90°, an enabled antenna at a served angle, and *the same geometry P12 fitted the constant on*, where P12 had measured 26×. What moved it was P10-perf's `next_fast_len` φ' resizing (512 → 270), **a change with no physics content whatsoever**, and nothing in the build could notice; the generalizable lesson is that a constant fitted to measurements is coupled to every input of those measurements, including ones nobody thinks of as inputs. (2) **The economics inverted:** post-FFT a probe leg costs 66 % of a full leg rather than ~18 %, making the pre-gate *strictly dominated* — 2.33× baseline returning the **coarse** leg, where computing at 2N and returning the **fine** leg costs 2.00×. Deletion made the formerly pre-gated Ka geometries **16× more accurate** (θ=5°: +0.0126 → +0.0008 dB) for +28 % work, and made `dsn_34m` X θ=45° **31 % cheaper** (there the pre-gate declined, so its probe leg was pure waste). P13 also **explained the `{0,1}` probe set by mechanism** — per-mode relative error tracks intra-mode cancellation `Cₘ = ∫\|Fₘ\|dρ / \|∫Fₘdρ\|`, not `\|Rₘ\|`, and `Cₘ` is largest at low `m` — and **corrected D17's record**, where both of P12's own §5 corrections turned out to be wrong (the "transposed labels" reading is falsified, and the "F7 floor masked it" explanation is falsified: the floor sits ~24 dB *above* every number in D17's table). See `docs/findings-2026-08-01-p13-pre-gate-retirement.md`. | **Decided + implemented 2026-07-31; P13 follow-up done 2026-08-01** | Measurement-driven, implementor 2026-07-31; recorded + follow-ups triaged 2026-08-01; P13 closed 2026-08-01 |
...
| D4 | Extract a shared `antenna-core` crate? | Split / keep two-crate layout | Split (mechanical move, after Phases 1–3). **✅ EXECUTED 2026-08-13.** The workspace is now three crates: `antenna-core` (the `model/` physics engine, the `data/{types,loader}` calibration-artifact layer, `error.rs`, `warnings.rs`), `antenna-model` (the REST service — `api/`, `service/`, `config/`, `data/repository.rs`, binaries), and `calibrate`. **Measured headline: calibrate's normal dependency tree went 9 → 1** on `poem\|h3o\|utoipa\|dashmap\|lru` — all eight web-stack lines are gone; the survivor is `lru`, which reaches calibrate through **its own** `aws-sdk-s3`, the same carve-out `tokio` gets (`cargo tree -p calibrate -e normal -i lru@0.16.4` proves it). **`antenna-model` is retained as a *dev*-dependency of `calibrate`, deliberately**: `calibrate/tests/**` serve generated artifacts through the real service path (`service::compute_gain_from_request`), which is what caught C13's 27.3 dB defect — dev-only is what keeps the shipped CLI web-free without giving up that coverage, and it must not be pruned as "unused". The split was import-transparent: `antenna-model`'s `lib.rs` re-exports `antenna_core::{error, model, warnings}` and its `data/mod.rs` re-exports `antenna_core::data::{loader, types}`, so **no test or bench import changed** and no test value moved (1052/1052 full tier; 892 `#[test]` functions workspace-wide at both `6f42799` and the merge commit, and `git diff` over `antenna-model/tests`, `antenna-model/benches`, `calibrate/tests` and `openapi.yaml` is empty). `Position3D`/`CoordinateSystem` moved to `antenna_core::model::coordinates_3d` and are re-exported by `api::schemas`; `openapi.yaml` is byte-identical (`tests/openapi_spec.rs`). The `utoipa` derives sit behind core's optional **`openapi` feature**, so a package-scoped calibrate build compiles no utoipa. `ndarray` was **already unified at 0.16.1** before the split (the old two-version finding was stale) and is removed outright from the service and core as a dead dep, along with `crc32fast`, `thiserror`, `anyhow` and `toml`; `num-complex` and `postcard` were demoted to dev-deps in `antenna-model`. **Two latent defects surfaced, both the same class — a property that held only because a workspace build unified a feature ON, and both now guarded in `scripts/check.sh` + CI:** core's `openapi`-OFF configuration was compiled by nothing (now `cargo clippy -p antenna-core --all-targets -- -D warnings`), and `calibrate` used `#[tokio::main]` and `EnvFilter` without declaring `tokio/{macros,rt-multi-thread}` or `tracing-subscriber/env-filter`, inheriting them from `antenna-model` (now declared, and guarded by a package-scoped `cargo build -p calibrate` plus a `cargo tree` assertion that fails if anything drags the web stack back in — verified by restoring `antenna-model` as a normal dep and watching it fail). `clippy -p calibrate --all-targets` deliberately does **not** substitute for that build: `--all-targets` pulls the dev-dependency back in and re-unifies the features. | **Decided + executed** | Maintainer, 2026-08-13 |
| D9 | Ship calibration `.bin` artifacts in-repo? | Commit binaries / generate in CI / docs-only | No binaries; document + script the generation path. **Note (2026-07-28, from C12/C8 stage 4):** `CalibrationMetadata.rmse_db`/`r_squared` stay plain `f64` with a NaN sentinel by decision — converting them to `Option<f64>` is an ANTC wire-format break that belongs with this unit's version-axes work (see D2), not the contract pass. **Note (2026-07-29):** unit D14 will provide this unit's worked exemplar — a real-anchored artifact produced by a documented script, generated locally and never committed. **✅ It does, as of 2026-08-02: `scripts/generate-cr159703-artifact.sh`** builds the NASA CR-159703 1.22 m artifact end to end from committed inputs (digitized peaks PSV + antenna-class YAML), writes it outside the repository tree, and prints the fabrication list and the "do not commit" reminder on every run; `calibrate/tests/cli_full_mode_real_data_e2e.rs` runs the same two binaries with the same arguments and then serves the result. Two things this unit still owns were measured rather than hypothetical: **C13** had to be fixed before any `.bin` could be served at all (−27.3 dB, see the Phase 3 row), and the artifact's reported cross-validation figure was not what it looked like (**D22** — ✅ closed 2026-08-03; the exemplar's folds now read 0.029–0.060 dB against an in-sample 0.027, where they read 0.12–10.86). **D23 also closed 2026-08-03**, which matters to this row specifically: it was the remaining blocker on shipping an artifact for `GroundStation_13m` or `UHF_Array_Element`, whose non-unity `asymmetry_factor` the artifact could not carry. **D21 closed 2026-08-04** and settles part of what this row still owes: a shipped artifact now *states* what it can promise off the main lobe rather than leaving it to a findings doc — `CalibrationMetadata.angular_resolution` carries the delivered knot spacing and the required `λ/D` lobe period on both angular axes, and the exemplar's own figures (0.577 cone / 0.119 clock knots per lobe period) say plainly that it carries the residual's envelope trend and not its lobe structure. What this row still owes is the decision in **D24**: whether that limitation is ever lifted, or is a permanent property of what published antenna data can support. **✅ DECIDED 2026-08-16 — docs-only, no binaries (the recommended default), and the unit is CLOSED.** The rationale is now recorded where it is actionable rather than only here: an artifact is a *build output* derived from measurements **plus this codebase's own physics model**, so a committed one keeps serving numbers after the model underneath it moves — and `PHYSICS_MODEL_VERSION` went 4 → 9 in a single month of hardening. Generate-in-CI was rejected as cost without a consumer (`calibrate`'s own tier is ~535 s and no test needs a CI-built artifact that a local run cannot produce). Exit criterion 2 was met by D14 and **re-verified end to end on 2026-08-16**: `scripts/generate-cr159703-artifact.sh` runs from a clean checkout, and its documented reference figures still reproduce exactly — corrected RMSE 0.0272 dB against model-only 11.0266, strided CV folds 0.029/0.031/0.031/0.060/0.046 dB, main lobe 0.011 dB and first sidelobe 0.019 dB both PASS, and D21's angular-resolution warning firing at cone 0.58 / clock 0.12 knots per lobe period. Exit criterion 3 turned out to be the work, and the premise had gone stale **twice**: this row's own "four uncalibrated antennas" note (corrected 2026-07-09 from "all disabled") was itself overtaken when `dsn_70m_uncalibrated` and `gbt_100m_uncalibrated` were added by the reference-validation work. Measured state at close: **10 entries, 5 enabled, 11 feeds**, verified against a running service. `test_simple`, a 5 m development fixture, was disabled in the shipped config — no test depends on it (the integration suite loads its own `tests/fixtures/test_antennas.yaml`), but it was appearing in every operator's `/status` antenna_ids. The four `enabled: false` entries are kept as **templates**, each comment retargeted at the generation path instead of the bare "Enable when calibration file exists". The `/health` and `/status` copy needed **no** change — S5 had already made it correct — so the unit pinned it rather than rewriting it. **The README sweep found rot far past this row's scope, all of it now fixed:** the feature list advertised "Auto-detection of ECEF vs Geodetic coordinates", deleted by C8 stage 2 and now a 400; the entire API section documented a pre-3D contract (`POST /api/v1/evaluate` with flat `azimuth_deg`/`elevation_deg`, and a response field `g_over_t_db` that does not exist — it is `gain_db`); the config section named `config/service.toml` and `antennas.toml`, neither of which exists (`config/service.yaml`, `calibration_data/antennas.yaml`), an error **CLAUDE.md repeated** and which is now fixed in both; and the uncalibrated-antenna example was TOML syntax with field names (`antenna_id`, `design_specs_path`) the YAML schema has never had. Every replacement payload is either copied from a drift-tested `examples/requests/*.json` or captured from a live service. **Filed D30** — the ECEF request examples are geometrically invalid and nothing could see it. | **Decided** | Maintainer, 2026-08-16 |
...
- ~~**Wide-angle Ka latency until P10-perf/F7 land** (2026-07-15): the asymmetric (coma) served
  ...
  `/heatmap` fans out to ~10⁵ points).~~ **Substantially reduced 2026-08-01 by P10-perf**, which
  made the mode path 2.4–7.4× cheaper (`dsn_34m` Ka θ=90°: 2135 → 559 ms measured in release)
  with no physics change. **What this retires:** the served-coverage half — a steered in-scope
  geometry can no longer plausibly exhaust S3's wall-clock budget and return a 504 instead of a
  gain (the repo's own witness went 22.3 s → 4.0 s). **What remains:** wide-angle Ka is still
  ~559 ms against a <100 ms p95 target, so a `/heatmap` fanning out to ~10⁵ points at those
  angles remains impractical. The cost profile has changed shape, which is the useful part: it is
  now ~85 % aperture-plane function evaluation (`powf` + `sqrt`s per point) rather than the
  `O(n_φ · M)` transform, so any further work is a different optimization — cheaper
  illumination/phase evaluation, or reuse of `g` across radial legs (measured memory-prohibitive:
  ~72 MB for the Ka θ=90° case against a 512 MB footprint target). Not currently filed as a unit;
  raise one if a consumer needs wide-angle Ka heatmaps.
...
- **The calibration pipeline's accuracy claims are now verified end-to-end, with caveats**
  ...
  (boresight, NTIA sweeps — landed 2026-07-31: tuned physics reproduces six published Andrew
  43998 gains at 0.0828 dB RMSE, and a second fixture drives the correction-carrying branch on
  real data) and ✅ D14 (real-anchored full-mode artifact — landed 2026-08-02: the NASA
  CR-159703 1.22 m hybrid fill is fitted by the real binary and **served through
  `compute_gain_from_request`**, the first full-mode artifact in the repo ever served; served
  boresight lands 0.0935 dB from the report's published 41.4 dBi, and 19 digitized peaks go from
  11.58 dB RMS uncorrected to 3.19 dB. It fixed **C13** on the way — a full-mode artifact's feed
  was being served a focal length off-focus, −27.3 dB — and filed **D21**, **D22** and, in its
  review pass, **D23**; ✅ D22 and D23 both closed 2026-08-03, leaving D21 as the only one of
  the three still open); the
  ...
  carrying all four. **Followed up 2026-07-31 by D17, which found the same class one seam
  further out and closed half of it.** The open item here was that nothing systematically
  checks that every stage of the calibrate pipeline evaluates the same `IntegrationParams`.
  D17 closed the **gate** axis structurally — one shared setter
  (`IntegrationParams::with_uncorrected_physics_gates`) is now the only way any of the four
  production call sites, in either crate, sets `apply_spillover`/`apply_sidelobe_floor`, and
  an end-to-end guard asserts calibrate's reported RMSE still describes the served gain. The
  **base preset** axis remains open and is the larger of the two: `calibrate` builds from
  `default()` and the service from `adaptive()`, measured 1.16 dB apart at θ=16° on D12's
  fixture geometry with `converged = true` and no warning, because the asymmetric
  azimuthal-mode path's self-check tests mode truncation only and never verifies radial
  density. That is a served-path convergence defect rather than a pipeline-consistency one,
  so D17 routed it to the P10 family. **Promoted to its own unit P12 on 2026-07-31** after
  re-measurement against the exact `antennas.yaml` parameters — see the risk entry below; the
  preset divergence turns out to be a *symptom*, and closing it is a side effect of P12's
  floor decision rather than the fix. The service side is otherwise unaffected throughout: the
  four enabled antennas are uncalibrated design-spec entries that load no correction surface.
- ~~**The correction surface returns ~0 across the topmost knot span of every axis**
  ...
  requirement). **This fix does not make the fit well-determined.** **Both were filed as units
  2026-08-02 — D19 and D20 — after re-measurement showed the first to be materially larger than
  recorded: the shipped 4/6/8 configuration puts adaptive knots *on* the frequency and clock
  bounds, giving end multiplicity `order + 1`, so B₀ and B_{n−1} are identically zero on those
  axes and **360 of the 960 coefficients (37.5 %) belong to basis functions that are zero
...
- **The azimuthal-mode integrator returns unverified radial quadrature as `converged = true`**
  ...
  **symmetric** branch verifies its radial density with an N-vs-2N recomputation
  (`model/integration.rs:514-530`); the **asymmetric** (azimuthal-mode) branch, taken by every
  laterally-offset or `asymmetry_factor != 1.0` feed, computes `n_rho` once at `:545` and
  checks only azimuthal mode truncation `I(M)` vs `I(M+1)` (`:550-554`). Radial convergence is
  therefore *assumed* on that branch and reported as verified. Measured 2026-07-31 against the
  exact `antennas.yaml` parameters, served `adaptive()` vs a radial ladder run to convergence,
  with `n_phi`/`m_max` held constant so every delta is purely radial: `gs_3.7m_uncalibrated` /
  `x_band_feed` at 8.4 GHz, θ=5° serves **−30.4500 dBi against −29.6343 converged (0.82 dB)**;
  `dsn_34m_uncalibrated` / `x_band` at 8.45 GHz, θ=0.10° serves **2.9760 against 4.1427
  (1.17 dB)**; D12's `UHF_Array_Element` fixture at 600 MHz, θ=16° serves **−50.7668 against
  −49.5383 (1.23 dB)**. All three report `converged = true` with no warning. **This is not
  latent on the enabled antennas** — D17's latency reasoning (their `D/λ ≈ 97–3600` puts the
  density floor's binding cone within a fraction of a degree of boresight) holds only for the
  floor, and the `gs_3.7m` row shows the budget is short **where the floor is not binding at
  all** (`radial_points_for` returns 43 there; floors 16 and 32 give identical answers, both
  0.82 dB wrong). So there are two sub-defects — a floor that is too low *and* a ~2× Nyquist
  budget that is insufficient on the mode path — sharing one root cause: nothing checks. Five
  of the enabled feeds take this branch. Two maintainer decisions are open (form of the radial
  check, which costs up to 3× on a path P10-perf exists to make cheaper; and whether
  `adaptive()`'s floor of 16 is simply wrong); **P12 blocks P10-perf** on correctness ordering.
  **✅ Closed 2026-07-31 by P12, and its own residue closed 2026-08-01 by P13.** P12 gave the
  mode path the check and the refinement; P13 then retired the two fitted constants P12's fix
  rested on (`RADIAL_PRE_GATE_SAFETY = 32`, `RADIAL_PROBE_MODES = {0,1}`) by deleting the
  pre-gate they served. The safety factor had already stopped bounding its quantity — measured
  **43.5×** where it passes, on `dsn_34m` Ka θ=90° — because P10-perf's physics-neutral `n_phi`
  resizing moved the ratio underneath it. `PHYSICS_MODEL_VERSION` 8.
- **The correction surface carries the residual's envelope trend, not its lobe structure, and
  whether that can ever be fixed is unknown** (D21, 2026-08-04; filed as unit **D24**). D21
  closed the *silence* — every full-mode artifact now records what its knots resolve against
  its own `λ/D`, and the exemplar's figures are 0.577 knots per lobe period in cone and
  **0.119 in clock**, both far below the 2.0 a periodic feature needs. It did **not** close the
  limitation, and the fix that both the filing and this roadmap called "the real fix" turns out
  to be neither proven nor currently testable. Three independent reasons, and the first is the
  one to remember: **D14's fill contains no lobe-scale residual structure by construction** —
  it is the model plus a weighted least-squares *quadratic* trend per half-plane — so a finer
  surface fitted to it would recover nothing, and there is no other dataset. The 19 digitized
  anchors that expose the gap are the test set, not fittable data. Second, since D20 an
  underdetermined fit is a hard error, so deriving the knots from `λ/D` would make `calibrate`
  **refuse to produce an artifact at all** for `dsn_34m` X-band and every other narrow-beam
  ground station — the antennas D9 exists to ship — because it would demand a 3D grid sampled
  at ~0.03° in cone, and D14's own register row records that full 3D G/T grids are essentially
  never published, judged a permanent constraint. Third, nobody has measured whether the
  structure is even the surface's to carry: residual lobe structure at this scale is as
  plausibly a lobe/null **position** mismatch (feed position, phase centre, surface phase) as a
  level error, and an additive smooth dB surface is the wrong instrument for a positional error
  at *any* resolution. D24 records the two preconditions and a cheap experiment that would
  settle the third — refit D14's anchors against a perturbed feed position and see whether the
...
- **A constant fitted to measurements is coupled to every input of those measurements** (the
  ...
  (P10-perf's `next_fast_len` φ' sizing, 512 → 270) moved that ratio to 43.5× on the same
  geometry, and **nothing in the build could detect it** — the constant guards a runtime
  certification, and no test asserted the margin it depends on. The same exposure exists wherever
  else this codebase carries an empirically-fitted numeric guard. Not currently filed as a unit;
  the concrete instance is closed, the class is not.
- ~~**`bessel_jn` is inaccurate exactly at its turning point `m ≈ x`, and nothing bounded it until
  now**~~ **✅ Closed 2026-08-01 by P14** (`PHYSICS_MODEL_VERSION` 9). The Miller start offset now
  scales with the turning-point width — `12·x^(1/3)`, the constant **derived** from an Airy decay
  requirement rather than fitted to the error table below — and closure at the turning point went
  from growing without bound in x to **~3e-16 flat**. `J₀`/`J₁` below |x| = 8 also moved from a
  rational fit (~3e-9 absolute, `J₀(0) = 1 + 2.83e-9`) to the convergent ascending series (~1e-14,
  exactly 1 at the origin). Served gain moves ~1e-7 dB; no reference anchor, oracle cross-check or
  convergence pin moved. **Three things worth carrying forward.** (i) The module's existing graders
  were recurrence identities, which are **scale-invariant** — and uniform mis-scaling is precisely
  how Miller's algorithm fails, so they could not have graded this fix at all. P14 built the
  independent grader the module never had (a compensated trapezoidal quadrature of Bessel's own
  integral) and it is committed; use it for any future change here. (ii) The filed table was
  measuring **two** ceilings at once: at `m = x` exactly, `J_{m−1}` sits on the *upward* branch,
  which is limited to ~3e-9 by the `J₀`/`J₁` asymptotic fit — a separate ceiling the identity
  cannot see, since an upward recurrence satisfies it by construction however wrong its seeds are.
  That branch was deliberately not replaced: the Hankel asymptotic's own smallest term at x = 8 is
  ~2e-8, so only a Chebyshev minimax fit could beat what is there, for ~2.6e-8 dB. (iii) The
  constant carries the margin test P13 said to write, and it is **measured, not argued** — re-run
  the shipped recurrence at 3× the offset and require the answer not to move, with a negative
  control asserting the pre-P14 flat 40 still *fails* that check. *History (the defect):* the
  downward (Miller) branch started a flat
  `acc = 40` orders above the wanted one — the very fixed-offset scheme `bessel.rs`'s own module
  header warns about, since the turning-point transition width grows like `x^(1/3)`. The
  2026-07 two-branch rework removed the problem for `m ≪ x` but left it *at* `m ≈ x`, where
  downward is the only stable direction. Measured closure of `(2m/x)·Jₘ = J_{m−1} + J_{m+1}` at
  `m = x`: **2e-8 at x=255, 2e-5 at x=1000, 9e-3 at x=10⁴** — while at `m = 0.9x` and `m = 1.1x`
  it closes to ~1e-15 throughout, so the defect is sharply localized rather than general.
  **Latent, and bounded by a single constant:** the only caller reaching that branch is the
  azimuthal-mode integrator, capped at `MODE_M_MAX = 254`, where the error is 2e-8 (~1.7e-7 dB).
  It becomes live the moment that cap is raised. Surfaced only because P10-perf's review minor
  (b) added the first high-order Bessel coverage the module has ever had — the pinned orders
  previously stopped at `m = 5`, which is why a routine this codebase depends on had an unmeasured
  accuracy cliff. It was pinned by `jn_turning_point_accuracy_degrades_far_above_the_served_order_ceiling`
  so it could not silently become relevant; P14 inverted that test into
  `jn_turning_point_closure_is_machine_precision_at_every_argument`, which is what the fix looks
  like from the test suite's side.

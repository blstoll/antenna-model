...
| **3 — API contract quality** | A client can trust the spec and the error contract — finalized once, then frozen. | One error vocabulary, JSON bodies, one status-code policy (C2–C4); `/h3-heatmap` documented in openapi + api-documentation with a verified example (C1, 2026-07-25); warnings no longer lost on a `/h3-heatmap` cache hit (C10, 2026-07-25); prose examples under a schema drift guard (C11, 2026-07-25); `loss_db` referenced to the grid peak on both heatmap endpoints (C9); **C8 contract finalization landed**: `feed_position` renamed, `coordinate_system` required, typed warnings, coherent heatmap endpoints; openapi drift guard (C7) in CI freezing the result. |

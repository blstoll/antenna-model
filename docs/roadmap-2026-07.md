...
| F7 | Statistical off-axis sidelobe envelope/floor model? (The physics model's sidelobes are systematically ~8–13 dB optimistic — see contract "Off-axis pattern / sidelobe fidelity".) | Implement envelope/floor model / docs + warning only / full physical modeling | Envelope/floor model, **data-gated**: requires reference sidelobe data (the ITU S.580 harness test validates pattern *shape* only — levels need real data). Near-term honesty warning is NOT gated (unit P8, decided with this row's filing). Physical edge-diffraction/strut modeling is out of scope regardless (§6). **P8 warning IMPLEMENTED 2026-07-12** (branch `feat/p8-off-axis-honesty-warning`). **F7 data gate MET 2026-07-12:** digitized reference sidelobe datasets committed — `ntia_84_164_sidelobe_statistics.psv` (22 C-band earth stations, 2.8–13 m, D/λ 35–267, statistical distributions across 1°–180°) and `nasa_cr159703_pattern_peaks.psv` (measured pattern cuts of 1.22/1.83 m prime-focus paraboloids at 12 GHz with surface-error provenance). The register row decision is still needed before starting F7. | Open (P8 done; data gate met) | Maintainer filed 2026-07-10 |

...
| **2 — Safety & operational correctness** ⚠️ **DONE 2026-07-23 with two open gaps (S1b, S2b)** | Config promises kept; bounded work; honest lifecycle. | Oversized → 413 (S1) — **but only when `content-length` is present; chunked bodies bypass it (S1b, open)**; slow → 504 timeout (S2) — **but unenforceable on `POST /api/v1/gain`, which never yields (S2b, open)**; ✅ integration has a wall-clock budget (S3); ✅ concurrency capped → 503 + Retry-After (S4, **ships disabled at `max_concurrent_heavy_requests: 0`**; permit release on timeout bounds admitted requests, not running compute); ✅ readiness/fail_fast/shutdown real (S5); ✅ H3 validator complete (S6). S7 superseded by C8. (Coordinate-ambiguity handling moved to C8, which removes the ambiguity instead of warning about it.) **Phase 2 flips to unqualified DONE when S1b + S2b land** — both found by the 2026-07-24 Phase 2 review, which ran in parallel with S5 and so missed the original closeout. |

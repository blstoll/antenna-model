...
against HEAD on 2026-07-07, and re-verified 2026-08-14 (roadmap D27 finding 8).

**Citation policy, set 2026-08-14 (roadmap D27 finding 8): cite a file and a *symbol*,
not a line number.** New citations must follow it, and existing ones convert as they are
touched. Line numbers have rotted twice here — once in the beam-steering fixes, and again
in D4's crate split — and the second rot is the instructive one.

D4 moved every `model/` file into `antenna-core`, but only `coordinates_3d.rs`'s citations
broke, because that file also *gained* `Position3D`/`CoordinateSystem` and was reordered.
Its 14 cited symbols had drifted by **seven different offsets** (+15, +21, +28, +88, +113,
+115, +154), so no mechanical correction was possible, and `compute_emitter_direction` and
`compute_emitter_direction_with_attitude` had **swapped order** — the citations were not
merely stale but misleading about the file's shape. All of them are now symbol-based.

The remaining line-number citations (`coordinates.rs`, `geometry.rs`, `pattern.rs`,
`integration.rs`, `illumination.rs`, `heatmap.rs`, `schemas.rs`, `evaluator.rs`) survived
D4 because those files moved wholesale without internal reordering; 10 were spot-checked on
2026-08-14 and every one still landed on the construct described. They are correct but
fragile, which is the point of the policy: a symbol name survives a move, and `grep` can
check it, which is what makes a disagreement between this file and the code *findable*
...
|---|---|---|---|---|
| ECEF | Earth-Centered Earth-Fixed, meters | Earth's center of mass | X: equator ∩ prime meridian; Y: equator at 90°E; Z: North pole; right-handed | `Position3D` with `coordinate_system: ecef` (required, never inferred); `geodetic_to_ecef`/`ecef_to_geodetic` (`antenna-core/src/model/coordinates_3d.rs`) |
...
| ENU (East-North-Up) | Local tangent-plane frame at a given (lat, lon) | The (lat, lon) point itself, not Earth's center | Rows of `R` are East, North, Up expressed in ECEF (documented on `ecef_to_enu_rotation`) | `ecef_to_enu_rotation` (`coordinates_3d.rs`); heatmap emitter-position generation (`service/heatmap.rs:318-362`) |
| Antenna frame | Cartesian, origin at the reflector vertex (≡ `vehicle_position`, see invariant below) | Reflector vertex / vehicle position (assumed coincident) | Z = boresight (vehicle → reflector_boresight, normalized); X = azimuth-zero reference (attitude quaternion body +X if supplied, else Earth-Z/East cross-product heuristic); Y completes right-hand system | `coordinates.rs` (feed math, vertex origin, `:28,31`); `compute_emitter_direction[_with_attitude]` (`coordinates_3d.rs`) |
| Far-field / E-clock-E-cone | θ = **polar angle from boresight** (0°=boresight, 90°=perpendicular), NOT horizon elevation; φ = azimuth, 0°=+X, 90°=+Y | Antenna frame origin | Right-handed spherical | `FarFieldCoordinates`, `EClockConeCoordinates` (`coordinates.rs:80,115`); `antenna_frame_to_spherical` (`coordinates_3d.rs`) |
...
`coordinates_3d.rs`) measures the emitter offset from **`vehicle_position`**. No
...
requirement is documented on `ecef_to_enu_rotation` itself; the heatmap path applies it correctly
...
|---|---|---|
| Geodetic → ECEF | `geodetic_to_ecef` (`coordinates_3d.rs`) | Round trip with `ecef_to_geodetic` is identity to ~1e-6 (test `test_ecef_to_geodetic_roundtrip`) |
| ECEF → Geodetic | `ecef_to_geodetic` (`coordinates_3d.rs`) | Pole-safe altitude branch (inside `ecef_to_geodetic`, uses `z/sin(lat)` form when `\|cos(lat)\| ≤ 1e-4`); round trip near poles has its own test (`test_ecef_to_geodetic_pole_with_altitude`) |
| ECEF → ENU (rotation matrix) | `ecef_to_enu_rotation` (`coordinates_3d.rs`) | `R` is orthogonal: `R · Rᵀ = I`. Asserted by `test_ecef_to_enu_rotation_is_orthogonal` (added with this contract). |
...
| Antenna-frame Cartesian → far-field spherical (az, el) | `antenna_frame_to_spherical` (`coordinates_3d.rs`) | `el` is polar angle, **always in [0°, 180°]**, never horizon-based; `az` from raw `atan2` is in (−180°, 180°] before normalization |
| Raw azimuth → normalized azimuth | `normalize_azimuth_deg` (`coordinates_3d.rs`) | Output always in [0°, 360°). Coverage ranges and B-spline knots assume this range — **any new az-producing code path that skips this normalization silently breaks coverage lookups** (finding #5 in `docs/review-findings-2026-06-10.md`, fixed in commit `43a74af`). Boundary values asserted by `test_normalize_azimuth_deg_boundaries` (added with this contract). |
| Uncorrected (az, el) → squint-corrected (az, el) | `apply_beam_squint_correction` (`coordinates_3d.rs`) / `squint_corrected_direction` (`coordinates_3d.rs`) | **Argument-order trap**: `squint_corrected_direction` takes `(operating_freq, pointing_freq)`; it calls `apply_beam_squint_correction` with `(pointing_freq, operating_freq)` — reversed (inside `squint_corrected_direction`, already commented in code). Corrected elevation is `≥ 0` by construction; corrected azimuth renormalized to [0,360). Pinned by `test_squint_corrected_direction_frequency_argument_order` (added with this contract). |
...
| `axial_defocus` (config field `axial_defocus_m`) | Deliberate axial defocus of the feed's phase center from the focal point — the explicit knob for intentionally defocused-feed studies, distinct from (and not compensated like) `phase_center_offset` | meters | Positive = away from the reflector vertex; default 0 (focused, no defocus loss) | **Added 2026-07-10 (roadmap P7).** Consumed in `integration.rs` as `axial = position.z − focal_length + axial_defocus` (`integration.rs:911` in the P10 Hankel integrand builder `aperture_plane_g`, plus the sibling sites at `:752`/`:981` — line refs re-verified 2026-07-18), driving the same quadratic (defocus) aperture-phase term that `phase_center_offset` drove pre-P7. Pinned by `test_axial_defocus_produces_defocus_loss` (`integration.rs:1031`, 5 cm costs >1 dB at 8.4 GHz) and `test_axial_defocus_m_reduces_gain_at_service_level` (`service/evaluator.rs`, `axial_defocus_m: 0.05` costs >0.5 dB end-to-end). Service-config-only knob — calibrate writers always stamp `axial_defocus_m: 0.0` into artifacts (not a fitted/calibrated quantity). Not to be confused with the separate, pre-existing `phase_center_offset_wavelengths` unit quirk in `calibrate/antenna_config.rs:63` (unrelated, out of scope here). |
| `surface_rms` | Reflector surface RMS deviation from ideal parabola, used in Ruze's equation | meters | Should be ≪ shortest operating wavelength; example configs 0.4mm–1.5mm | **Scope (confirmed 2026-07-07):** the Ruze form `η = exp(-(4π·σ/λ)²)` (Ruze 1966) models **surface-error (roughness) efficiency only** — the boresight-gain loss from random deviations of the real dish from an ideal paraboloid. It is one multiplicative factor in the live gain path: `overall_efficiency` computes `eta_ruze * eta_mesh` (`pattern.rs:128-141`) using `pattern::ruze_efficiency` (`pattern.rs:112`). It is *not* the steering / off-boresight physics (that lives in the aperture-integration / phase model). The `4π` constant is `2·(2π/λ)`, the factor of 2 coming from the reflection double-pass — a correctly-handled reflected path error. Not independently re-derived against the primary reference; scope and constant confirmed self-consistent with the code. **Resolved (verified 2026-07-18, roadmap P6):** the duplicate Ruze implementation flagged here previously lived in `surface.rs`; that file has been deleted and `pattern.rs::ruze_efficiency` is the single implementation (grep-verified: no other definition in any `.rs` file). |
| `mesh_spacing` / `wire_diameter` | Wire-mesh reflector geometry; mesh introduces frequency-dependent reflection loss | meters | spacing ~1-10mm, wire diameter ~0.05-1mm, wire_diameter < spacing (enforced with an error, `geometry.rs:411-419`) | `pattern.rs::overall_efficiency` calls `mesh::mesh_reflection_efficiency` (inductive-grid model) directly, and since roadmap **D8** (2026-08-16) that is the *only* mesh implementation in the tree: `MeshParameters::transparency_at_wavelength`, a separate simpler low-frequency approximation reachable only from its own unit tests, was removed. No live-path value moved — the P1 staged-spillover decision had deliberately never wired it. |
| `f_over_d` (focal length / diameter) | Reflector geometry ratio, affects illumination/aperture efficiency and beam deviation factor | dimensionless | Typical [0.3, 0.5]; enforced range [0.2, 1.0] (`model/geometry.rs::{F_OVER_D_MIN, F_OVER_D_MAX}`) | **Fails loudly (roadmap P4, 2026-07-17):** out-of-range f/D is rejected with a typed error at every validation seam — model `ReflectorGeometry::validate` (formerly a silent no-op branch), calibration-artifact load (`data/types.rs`), inline design-spec config (`config/settings.rs`), and calibrate's design-spec / antenna-class loaders — all referencing the same canonical constants. Pre-P4 the loaders disagreed (ranges (0, 2.0], [0.2, 2.0], (0, 1.0) at different seams) and the model check was a no-op. |
...
| `temperature_k` (H3 link-budget requests) | System noise temperature forming G/T = G(dB) − 10·log₁₀(T) | Kelvin | Optional; user-supplied | **T is a pure user-supplied passthrough — no antenna noise-temperature model exists (sky/ground pickup, spillover noise = roadmap F4, undecided).** One shared formula since P5 (2026-07-17): `pattern::g_over_t_from_gain_db`, used by both `pattern::compute_g_over_t` and the H3 per-cell path (whose gain includes the correction surface). The H3 validator does not yet bound `temperature_k` (a non-positive value reaches `log10` → NaN) — closing that is roadmap S6, not P5. |
...
| Statement | Testable? |
|---|---|
| `geodetic_to_ecef` ∘ `ecef_to_geodetic` ≈ identity (away from poles, and at poles via the z/sin(lat) branch) | Yes — `test_ecef_to_geodetic_roundtrip` and `test_ecef_to_geodetic_pole_with_altitude` |
...
| `feed_pointing_location` API field is an Earth aim point, not a fixed physical offset: same `feed_pointing_location` + different `vehicle_position` ⇒ different physical feed displacement | Yes — `feed_pointing_location_is_an_aim_point.rs` (added with this contract; renamed from `feed_position_is_pointing_target.rs` by C8 stage 1) |
...
| Elevation/E-cone (polar angle) is always in [0°, 180°], never negative | Partially — `apply_beam_squint_correction` guards `≥ 0` via `debug_assert!` **and** a release-mode `.abs()` (both in
`apply_beam_squint_correction`); `antenna_frame_to_spherical` relies on `acos` range (mathematically [0,π]), not asserted at the boundary. **On the calibrate side this is now enforced at the boundary** (roadmap D26, 2026-08-13): measurement CSVs may record a *signed* E-cone (a one-sided pattern cut), so the parser reflects `(φ, −θ)` onto the identical direction `(φ + 180°, θ)` — `MeasurementPoint::to_polar_convention` — and `export_full_calibration` **refuses** an out-of-convention elevation extent instead of clamping it. Pinned by `calibrate/tests/negative_cone_served_coverage_e2e.rs`, including that the reflection does not move the physics (3.2e-6 dB on the asymmetric branch). |
...
| A quaternion passed as `vehicle_attitude` must be unit-norm; `quaternion_rotate` preserves vector length only for unit-norm input | Yes — `quaternion_rotate`'s doc comment states the assumption; confirm `validate_gain_request`/`validate_h3_link_budget_request` reject non-unit quaternions |
| Reflector boresight position must not coincide with vehicle position | Yes — enforced with an error in `compute_emitter_direction_with_attitude`, which
`compute_emitter_direction` delegates to |
...
`feed_pointing_location` (a pointing target) and the `reflector_boresight`, mapped to a physical
...
- **Resolved 2026-07-10 (roadmap P7, implemented).** `phase_center_offset` is now a
  ...
  an open item.
- **Resolved (2026-08-16, roadmap D8).** `MeshParameters::transparency_at_wavelength` — a
  simplified low-frequency mesh approximation whose only callers were its own unit tests —
  has been **removed**, along with `test_mesh_transparency` (grep-verified: zero hits in any
  `.rs` file). The P1 staged-spillover decision deliberately never wired it, so nothing on
  the live path changed. `mesh::mesh_reflection_efficiency`, called from
  `pattern.rs::overall_efficiency`, remains the single mesh implementation. Recorded here as
  history, not as an open item.
- **Resolved (verified 2026-07-18, roadmap P6).** The duplicate Ruze implementation
  (`surface.rs::ruze_efficiency` / `ruze_efficiency_from_frequency`) is gone: `surface.rs`
  has been deleted, leaving `pattern.rs::ruze_efficiency` as the single canonical
  implementation on the live path — see the `surface_rms` glossary entry. Recorded here as
  history, not as an open item.
- **Resolved 2026-07-17 (roadmap P4, implemented).** `f_over_d` out-of-range was a silent
  no-op in `model/geometry.rs::validate`; it now fails loudly with a typed error at every
  validation seam, all sharing the canonical `F_OVER_D_MIN`/`F_OVER_D_MAX` constants
  ([0.2, 1.0]) — see the `f_over_d` glossary row. Recorded here as history, not as an
  open item.
- **Resolved (verified 2026-07-18, roadmap P6).** The design-doc / code formula parity
  process item is closed: `docs/antenna-model-design-doc.md` §2.5 now documents both the
  beam-steering sign flip ("a lateral feed displacement steers the beam to the OPPOSITE
  side … hence the negative x/y") and the BDF division, matching
  `coordinates.rs::to_feed_displacement_with_bdf` (negated x/y, `displacement =

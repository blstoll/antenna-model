...
> - **[Minor] `num_evaluations` undercounts the asymmetric path** (reports `n_rho*n_phi`, omits ×M) — cosmetic metric only. (Folded into P10-perf.)
>
> **Peer-review pass (2026-07-15, addressed in `8366f79`):** a second reviewer raised four items. Three were fixed in `8366f79`: **(1)** `mode_count_for` ignored illumination asymmetry — a centered feed with `asymmetry_factor≠1` (cos(2φ') illumination, m≈±2 content) hit the `spread≤0` branch → `m_max=1`, both under-resolving AND spuriously flagging non-converged; now floors `m_max≥6` (combined via max with the coma bandwidth) when `asymmetry_factor≠1`, verified converged + <0.1 dB vs the 2D reference. **(2)** the 2% `HANKEL_SELF_CHECK_RTOL` was applied to the mode M-vs-(M+1) truncation check where its Richardson `diff/15` justification doesn't hold; added a separate `MODE_SELF_CHECK_RTOL=0.5%` (honest geometric-tail bound ⇒ ≤~0.043 dB, inside the <0.1 dB budget). **(4)** documented that `adaptive()`'s `min/max_phi_points`/`max_iterations` are inert on the served path. The 4th item (O(n_ρ·n_φ·M) latency) is the same as **P10-perf** above (independently confirmed).
